...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/abricos.avitoautoload/export_avito.php');
...
$APPLICATION->SetTitle(GetMessage('AVITO_DETAIL_TITLE'));
...
{
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
	ShowError(GetMessage('AVITO_ERR_NO_ACCESS_EXPORT'));
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
	die();
}
...
{
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
	ShowError(GetMessage("AVITO_ERR_NO_IBLOCK_CHOSEN"));
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
	die();
}
...
if (($arIBlock = $rsIBlocks->Fetch()))
{
	...
	{
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
		ShowError(GetMessage('AVITO_ERR_NO_ACCESS_IBLOCK'));
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
		die();
	}
}
...
{
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
	ShowError(str_replace('#ID#',$intIBlockID,GetMessage("AVITO_ERR_NO_IBLOCK_FOUND_EXT")));
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
	die();
}
...
if (!empty($arOffers['IBLOCK_ID']))
{
	...
	if ($arOfferIBlock = $rsOfferIBlocks->Fetch())
	{
		...
		{
			require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
			ShowError(GetMessage('AVITO_ERR_NO_ACCESS_IBLOCK_SKU'));
			require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
			die();
		}
	}
	...
	{
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
		ShowError(str_replace('#ID#',$intIBlockID,GetMessage("AVITO_ERR_NO_IBLOCK_SKU_FOUND")));
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
		die();
	}
	$boolOffers = true;
...
$arCondSelectProp = array(
	'ZERO' => GetMessage('AVITO_SKU_EXPORT_PROP_SELECT_ZERO'),
	'NONZERO' => GetMessage('AVITO_SKU_EXPORT_PROP_SELECT_NONZERO'),
	'EQUAL' => GetMessage('AVITO_SKU_EXPORT_PROP_SELECT_EQUAL'),
	'NONEQUAL' => GetMessage('AVITO_SKU_EXPORT_PROP_SELECT_NONEQUAL'),
);
...
$vatRates = array(
	'-' => GetMessage('AVITO_BASE_VAT_EMPTY'),
	'0%' => '0%',
	'10%' => '10%',
	'18%' => '18%'
);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if (!empty($_REQUEST['save']))
	{
		...
		if ($boolOffers)
		{
			...
			{
				$arErrors[] = GetMessage('AVITO_SKU_EXPORT_ERR_CONDITION_ABSENT');
			}
			if (YANDEX_SKU_EXPORT_PROP == $arSKUExport['SKU_EXPORT_COND'])
			{
				...
				{
					$arErrors[] = GetMessage('AVITO_SKU_EXPORT_ERR_PROPERTY_ABSENT');
					$boolCheck = false;
				}
				if ($boolCheck)
				{
					...
					{
						$arErrors[] = GetMessage('AVITO_SKU_EXPORT_ERR_PROPERTY_COND_ABSENT');
						$boolCheck = false;
					}
				}
				if ($boolCheck)
				{
					...
					if ($strPropCond == 'EQUAL' || $strPropCond == 'NONEQUAL')
					{
						...
						{
							$arErrors[] = GetMessage('AVITO_SKU_EXPORT_ERR_PROPERTY_VALUES_ABSENT');
							$boolCheck = false;
						}
		...
		if (isset($_POST['USE_VAT_EXPORT']) && is_string($_POST['USE_VAT_EXPORT']))
		{
			...
			if ($vatExport['ENABLE'] == 'Y')
			{
				...
				{
					$arErrors[] = GetMessage('AVITO_VAT_ERR_BASE_VAT_ABSENT');
					$boolCheck = false;
				}
			}
		}
...
<?
		...
		else
		{
			...
			$message = new CAdminMessage(GetMessage("AVITO_SAVE_ERR"), $e);
			echo $message->Show();
		}
	}
	else
	{
		$aTabs = array(
			array("DIV" => "AVITO-settings-types", "TAB" => GetMessage('AVITO_TAB1_TITLE'), "TITLE" => GetMessage('AVITO_TAB1_DESC')),
			array("DIV" => "AVITO-settings-prices", "TAB" => GetMessage('AVITO_TAB2_TITLE'), "TITLE" => GetMessage('AVITO_TAB2_DESC')),
		...
		{
			return '<tr id="AVITO_params_tbl_'.$intCount.'">
...
****************************************************************************/
		...
		<form name="AVITO_form" method="POST" action="<?=$APPLICATION->GetCurPage(); ?>">
...
?>
		...
		<tr>
			...
			<tr class="heading">
				<td colspan="2"><?=GetMessage('BX_CATALOG_EXPORT_AVITO_COMMON_FIELDS')?></td>
			</tr>
			<tr>
				<td colspan="2">
			<div style="padding: 10px;">
				<table width="90%" class="inner" style="text-align: center;">
					<tr>
						<td align="right"><?=GetMessage('BX_CATALOG_EXPORT_AVITO_DESCRIPTION'); ?></td>
						<td style="white-space: nowrap;"><select name="COMMON_FIELDS[DESCRIPTION]">
								<option value="PREVIEW_TEXT"<?=($commonFields['DESCRIPTION'] == 'PREVIEW_TEXT' ? ' selected' : '');?>><?=GetMessage('BX_CATALOG_EXPORT_AVITO_DESCRIPTION_PREVIEW_TEXT'); ?></option>
								<option value="DETAIL_TEXT"<?=($commonFields['DESCRIPTION'] == 'DETAIL_TEXT' ? ' selected' : '');?>><?=GetMessage('BX_CATALOG_EXPORT_AVITO_DESCRIPTION_DETAIL_TEXT'); ?></option>
							</select></td>
					</tr>
				</table>
			</div>
		...
		<tr class="heading">
			<td colspan="2" valign="top"><?=GetMessage('AVITO_PROPS_ADDITIONAL')?></td>
		</tr>
		<tr>
			<td colspan="2">
				<div id="config_param" style="padding: 10px auto; text-align: center;">
				<table class="inner" id="AVITO_params_tbl" style="text-align: center; margin: 0 auto;">
					<thead>
					<tr><td style="text-align: center;"> </td>
					<td style="text-align: center;"><? echo GetMessage('AVITO_PARAMS_TITLE'); ?></td>
					</tr>
					</thead>
					<tbody>
						<?
				...
				<div style="width: 100%; text-align: center;"><input type="button" onclick="__addYP(); return false;" name="AVITO_params_add" value="<? echo GetMessage('AVITO_PROPS_ADDITIONAL_MORE'); ?>"></div>
				</div>
<script type="text/javascript">
...
function __addYP()
{
	...
	newRow.id = 'AVITO_params_tbl_'+id;
...
?>
			<tr class="heading">
				<td colspan="2"><? echo GetMessage('AVITO_SKU_SETTINGS');?></td>
			</tr>
			<tr>
			<td valign="top"><? echo GetMessage('AVITO_OFFERS_SELECT') ?></td><td><?
			$arOffersSelect = array(
				0 => '--- '.ToLower(GetMessage('AVITO_OFFERS_SELECT')).' ---',
				YANDEX_SKU_EXPORT_ALL => GetMessage('AVITO_SKU_EXPORT_ALL_TITLE'),
				YANDEX_SKU_EXPORT_MIN_PRICE => GetMessage('AVITO_SKU_EXPORT_MIN_PRICE_TITLE'),
			);
			if (!empty($arSelectOfferProps))
			{
				$arOffersSelect[YANDEX_SKU_EXPORT_PROP] = GetMessage('AVITO_SKU_EXPORT_PROP_TITLE');
			}
			...
			if (!empty($arSelectOfferProps))
			{
				...
				<tr class="heading">
					<td><? echo GetMessage('AVITO_SKU_EXPORT_PROP_ID'); ?></td>
					<td><? echo GetMessage('AVITO_SKU_EXPORT_PROP_COND'); ?></td>
					<td><? echo GetMessage('AVITO_SKU_EXPORT_PROP_VALUE'); ?></td>
				</tr>
				<tr>
					<td valign="top"><select name="SKU_PROP_COND" id="SKU_PROP_COND">
					<option value="0" <? echo (empty($arSKUExport['SKU_PROP_COND']) ? 'selected' : ''); ?>><? echo GetMessage('AVITO_SKU_EXPORT_PROP_EMPTY') ?></option>
					<?
					...
					?></select></td>
					<td valign="top"><select name="SKU_PROP_SELECT" id="SKU_PROP_SELECT"><option value="">--- <? echo ToLower(GetMessage('AVITO_SKU_EXPORT_PROP_COND')); ?> ---</option><?
...
?>
	<tr class="heading">
		<td colspan="2"><?=GetMessage('AVITO_PRICES')?></td>
	</tr>

	<tr>
		<td><?=GetMessage('AVITO_PRICE_TYPE');?>: </td>
		<td><br /><select name="PRICE">
			<option value=""<? echo ($PRICE == "" || $PRICE == 0 ? ' selected' : '');?>><?=GetMessage('AVITO_PRICE_TYPE_NONE');?></option>
<?
	if (!empty($arGroups))
	{
