	...
	if(is_dir($dir)){
		foreach(glob($dir."/*") as $file){
			if(is_dir($file)){
				...
				rename($file,$_SERVER['DOCUMENT_ROOT']."/upload/strllog_novosti_export_files/".$basename);
			echo $file. "<br>";
			}
		}
	} else {
...
mvdir($_SERVER['DOCUMENT_ROOT']."/upload/strllog_novosti_export_files");
...
if ( isset($_REQUEST['ORDER_ID']) || isset($_REQUEST['ORDER_ID_TEXT']) )
{
   ...
   if ( $nOrder && CSaleOrder::GetByID($nOrder) )
   {
      ...
      header('Content-Type: application/x-unknown');
	   	...
	   	include($_SERVER['DOCUMENT_ROOT']."/local/components/bitrix/sale.export.1c/CCustomSaleExport.php");
		$export = new CCustomSaleExport();
     	 ob_start();
     	$export::ExportOrders2Xml(array('ID' => $nOrder));
		$sXml = ob_get_clean();
      ...
      <tr>
         <td class="content-cell">
            <div class="center-content">
               ...
               <form action="?mode=query" method="post" style="line-height: 24px;">
