...
use Caweb\Main\Secret\MyLittleHelper;

class cmlImport  extends \CIBlockCMLImport {
    ...
    function ImportElementPrices($arXMLElement, &$counter, $arParent = false)
    {
        ...
        /**присваиваем всем торговым предложениям цену основного товара*/
        $arOffers = \CCatalogSku::getOffersList($arElement['ID'],0,array(), array('XML_ID'))[$arElement['ID']];
        if ($arOffers){
            foreach ($arOffers as $arOffer){
                $arXMLOffer = array();
                $arXMLOffer = $arXMLElement;
                $arXMLOffer[$this->mess['IBLOCK_XML2_ID']] = $arOffer['XML_ID'];
                $this->ImportElementPrices($arXMLOffer, $counter);
            }
        }
    ...
    protected function setPriceGroupProperty(&$properties, $groupValue) {
        ...
        foreach ($property as $value){
            if (!is_array($value) && (!key_exists('VALUE', $value)))
                continue;
            elseif((int)$value['VALUE'] === $enumId) {
                $oldValue = true;
                break;
            }
        }
        if ($newValue === $oldValue) return;
        $newArray = array('n0' => array('VALUE' => null, 'DESCRIPTION' => null));
        if ($newValue)
