...
if ('POST' == $_SERVER['REQUEST_METHOD'] && (strlen($save)>0 || strlen($apply)>0) && check_bitrix_sessid())
{
	...
	$arFields = array(
        ...
        'ACTIVE_TO' => (isset($_POST['ACTIVE_FROM']) ? $_POST['ACTIVE_FROM'] : '')
	);

	$DB->StartTransaction();
	if (0 < $ID)
	{
		$bVarsFromForm = !DiscountTable::update($ID, $arFields)->isSuccess();
	}
	else
	{
		$ID = DiscountTable::add($arFields);
	...
	if (!$bVarsFromForm)
	{
		$DB->Commit();
		if (strlen($save)>0)
			LocalRedirect("caweb_discount_edit?lang=".LANGUAGE_ID);
		elseif (strlen($apply)>0)
			LocalRedirect("caweb_discount_edit?lang=".LANGUAGE_ID.'&ID='.$ID);
	}
	else
	{
		if ($ex = $APPLICATION->GetException())
