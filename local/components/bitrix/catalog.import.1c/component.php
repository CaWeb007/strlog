...
if(isset($_GET) && !empty($_GET)) {
		...
		$fps = fopen($_SERVER['DOCUMENT_ROOT'].'/logs/querys-'.(date("d.m.Y")).'.txt', "a");
		
		foreach ($_GET as $arrkey=>$data )
			{
			$qat_date = date('l jS \of F Y h:i:s A');
			//$qat_date = date('F Y h:i:s A');
			$result = fwrite($fps,$qat_date."  ". $arrkey."   ".$data."   SessionId: ".session_id()."\n");
			}
		fclose($fps);
}
*/
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	elseif ($NS["STEP"] == 9)
    {
        ...
        $strMessage = "Import Product Sets OK";
        $NS["STEP"]++;
    }
	else
	{
