...
class CCustomSaleExport extends \CSaleExport {
	...
	{
		ob_start();
		static::ExportContragents($arOrder, $arProp, $agent, $arOptions);
	...
	static function ExportContragents($arOrder = array(), $arProp = array(), $agent = array(), $arOptions = array())
	{
		...
		<?
		$agent['REKV']['REKV_0'] = trim($agent['REKV']['REKV_0']);
		$xmlIdTemp = htmlspecialcharsbx(substr($arOrder["USER_ID"].'U'.$agent['REKV']['REKV_0']."#".$arProp["CRM"]["CLIENT"]["LOGIN"]."#".$arProp["CRM"]["CLIENT"]["LAST_NAME"]." ".$arProp["CRM"]["CLIENT"]["NAME"]." ".$arProp["CRM"]["CLIENT"]["SECOND_NAME"], 0, 40));

		if ($bExportFromCrm):
			$xmlId = htmlspecialcharsbx(substr($arOrder["USER_ID"].'U'.$agent['REKV']['REKV_0']."#".$arProp["CRM"]["CLIENT"]["LOGIN"]."#".$arProp["CRM"]["CLIENT"]["LAST_NAME"]." ".$arProp["CRM"]["CLIENT"]["NAME"]." ".$arProp["CRM"]["CLIENT"]["SECOND_NAME"], 0, 40));
		else:
				if(strlen($arOrder["SALE_INTERNALS_ORDER_USER_XML_ID"])>0):
                    {	
						if($xmlIdTemp != $arOrder["SALE_INTERNALS_ORDER_USER_XML_ID"]){
							$exp = explode("#",$arOrder["SALE_INTERNALS_ORDER_USER_XML_ID"]);
							$exp[0] = $arOrder["USER_ID"].'U'.$agent['REKV']['REKV_0'];
							$arOrder["SALE_INTERNALS_ORDER_USER_XML_ID"] = implode("#",$exp);
							$xmlId = htmlspecialcharsbx($arOrder["SALE_INTERNALS_ORDER_USER_XML_ID"]);
						}
					}
                else:
				    $xmlId = htmlspecialcharsbx(substr($arOrder["USER_ID"].'U'.$agent['REKV']['REKV_0']."#".$arProp["USER"]["LOGIN"]."#".$arProp["USER"]["LAST_NAME"]." ".$arProp["USER"]["NAME"]." ".$arProp["USER"]["SECOND_NAME"], 0, 40));
