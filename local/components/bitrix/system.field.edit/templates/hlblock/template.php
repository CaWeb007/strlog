...
if($arParams['arUserField']["SETTINGS"]["DISPLAY"] != "CHECKBOX")
{
	...
	{
		?>
<select name="<?echo $arParams["arUserField"]["FIELD_NAME"]?>" size="<?echo $arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"]?>" <?=($arParams["arUserField"]["EDIT_IN_LIST"]!="Y"? ' disabled="disabled" ':'')?> <?if($arParams["arUserField"]["MANDATORY"] == "Y"):?>required<?endif?>>
			<option value=" " title="Выбрать">-- Выбрать --</option>
		<?
		$bWasSelect = false;
