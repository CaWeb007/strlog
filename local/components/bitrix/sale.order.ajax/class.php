...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		$this->arResult = array(
			...
			'CURRENT_BUDGET_MAX_FORMATED' => false,
			'CURRENT_BUDGET_MAX' => false,
			'DISCOUNTS' => array(),
			'AUTH' => array(),
	...
	protected function registerAndLogIn($userProps)
	{
		...
		/* set a password to send the user */
		global $GLOBAL;
		$GLOBAL['NUP']['VALUE'] = $userData['NEW_PASSWORD'];
		/* END set a password to send the user */
	...
	protected function obtainPaySystem()
	{
		...
		{
			$arResult['PAYED_FROM_ACCOUNT'] = $innerPayment->getSum();
	...
	protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
	{
		...
		if ($this->arParams['PAY_FROM_ACCOUNT'] === 'Y' && $order->isAllowPay())
		{
			...
			/* Кастом */
			$BonusPayRestriction = Sale\Services\PaySystem\Restrictions\Manager::getClassesList()[0];
			
			$params = array(
				'select' => array('CLASS_NAME', 'PARAMS'),
				'filter' => array(
					'SERVICE_ID' => $innerPaySystemId,
					'=CLASS_NAME' => $BonusPayRestriction
				)
			);

			$dbRes = Sale\Services\PaySystem\Restrictions\Manager::getList($params);
			
			if ($data = $dbRes->fetch()) {
				 $restrictionParams = $data['PARAMS'];
			}
			
			$BPR = new $BonusPayRestriction();
			$BPR->setParams($restrictionParams);
			
			$sumRange = $BPR::getRange($innerPayment, $restrictionParams);
			
			$request = Bitrix\Main\Application::getInstance()->getContext()->getRequest();
			$PAY_USER_BUDGET_VALUE = $request->get("PAY_CURRENT_ACCOUNT_VALUE");
			...
			$maxBudgetValue = $sumToSpend;
			
			if($PAY_USER_BUDGET_VALUE && $PAY_USER_BUDGET_VALUE <= $userBudget){
				$maxBudgetValue = $sumToSpend;
				$sumToSpend = $PAY_USER_BUDGET_VALUE;
			}
			...
			if (!empty($arResult['USER_ACCOUNT']) && $sumToSpend > 0)
			{
				...
				//var_dump("FFFF",$arPaySystemServices);exit;
		...
		if ($sumToSpend > 0)
		{
			...
			$arResult['CURRENT_BUDGET_MAX'] = $maxBudgetValue;
			$arResult['CURRENT_BUDGET_MAX_FORMATED'] = SaleFormatCurrency($maxBudgetValue, $order->getCurrency());
		...
		else
		{
			...
			if(isset($arResult['CURRENT_BUDGET_MAX_FORMATED'])) unset($arResult['PAY_FROM_ACCOUNT_MAX_FORMATED']);
			if(isset($arResult['CURRENT_BUDGET_MAX'])) unset($arResult['PAY_FROM_ACCOUNT_MAX']);
		}
	...
	protected function getJsDataResult()
	{
		...
		$result['CURRENT_BUDGET_MAX'] = $arResult["CURRENT_BUDGET_MAX"];
		$result['CURRENT_BUDGET_MAX_FORMATED'] = $arResult["CURRENT_BUDGET_MAX_FORMATED"];
		...
		$result['TOTAL'] = array(
			...
			'PAYED_FROM_ACCOUNT' => $arResult["PAYED_FROM_ACCOUNT"],
