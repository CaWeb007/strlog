...
class CSaleGiftBasketComponent extends CCatalogViewedProductsComponent
{
	...
	/** @var array */
	private $productIds;
    private $cawebCustomPrice = false;
	/**
	...
	public function onPrepareComponentParams($params)
	{
        ...
        if (is_array($params['FULL_DISCOUNT_LIST']))
            foreach ($params['FULL_DISCOUNT_LIST'] as &$discountCond)
                foreach ($discountCond['ACTIONS']['CHILDREN'] as &$children)
                    if ($children['CLASS_ID'] === 'CawebGiftCondGroup'){
                        $children['CLASS_ID'] = 'GiftCondGroup';
                        $this->cawebCustomPrice = true;
                    }

        if (is_array($params['APPLIED_DISCOUNT_LIST']))
            foreach ($params['APPLIED_DISCOUNT_LIST'] as $key => &$discountCond)
                foreach ($discountCond['ACTIONS']['CHILDREN'] as &$children)
                    if ($children['CLASS_ID'] === 'CawebGiftCondGroup'){
                        $children['CLASS_ID'] = 'GiftCondGroup';
                        unset($params['FULL_DISCOUNT_LIST'][$key]);
                    }
		return $params;
	}
	...
	{
        $newPrice = 0;
        if ($this->cawebCustomPrice) {
            $newPrice = 1;
            $offer['MIN_PRICE']['PRINT_DISCOUNT_DIFF'] = CCurrencyLang::CurrencyFormat(1, 'RUB');
        }
        ...
        $offer['MIN_PRICE']['VALUE'] = $offer['MIN_PRICE']['VALUE_VAT'];
        $offer['MIN_PRICE']['PRINT_VALUE'] = $offer['MIN_PRICE']['PRINT_VALUE_VAT'];
		$offer['MIN_PRICE']['DISCOUNT_DIFF'] = $offer['MIN_PRICE']['VALUE_VAT'];
		$offer['MIN_PRICE']['DISCOUNT_DIFF'] = $offer['MIN_PRICE']['PRINT_VALUE_VAT'];
		$offer['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] = 100;
		$offer['MIN_PRICE']['DISCOUNT_VALUE_NOVAT'] = $newPrice;
		$offer['MIN_PRICE']['DISCOUNT_VALUE_VAT'] = $newPrice;
		$offer['MIN_PRICE']['DISCOUNT_VALUE'] = $newPrice;
	}
