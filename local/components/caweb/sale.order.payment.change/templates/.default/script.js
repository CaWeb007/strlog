BX.Sale.OrderPaymentChange = (function()
{
	var classDescription = function(params)
	{
		...
		this.priceGroupRefresh = params.priceGroupRefresh || [];
		this.oldPaySystemId = params.oldPaySystemId || '';
				...
				BX.ajax(
					{
						...
						{
							if(this.priceGroupRefresh.includes(hidden.value) || this.priceGroupRefresh.includes(this.oldPaySystemId)){
								alert('Возможен перерасчет стоимости заказа. Страница будет перезагружена');//todo как то по идее нужно оповещать
