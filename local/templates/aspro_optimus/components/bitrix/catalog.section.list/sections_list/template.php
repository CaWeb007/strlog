...
<div class="catalog_section_list rows_block items">	
	...
	?>
		<?
        $elementsQuantity = CIBlockSection::GetSectionElementsCount($arItems["ID"], Array("CNT_ACTIVE"=>"Y"));
        ?>
				...
				<table class="section_item_inner">	
					<tr>
						...
						<td class="section_info">
							<ul>
								<li class="name">
									<a href="<?=$arItems["SECTION_PAGE_URL"]?>"><span><?=$arItems["NAME"]?><i> <?=$elementsQuantity;?></i></span></a> 
								</li>
								<?if($arItems["SECTIONS"]){
									foreach( $arItems["SECTIONS"] as $arItem ){?>
										<li class="sect"><a href="<?=$arItem["SECTION_PAGE_URL"]?>" class="dark_link"><?=$arItem["NAME"]?><? echo $arItem["ELEMENT_CNT"]?'&nbsp;<span style="color:#00adee !important;">'.$arItem["ELEMENT_CNT"].'</span>':'';?></a></li>
									<?}
								}?>
							</ul>
						</td>
					...
					<?if($arParams["SECTIONS_LIST_PREVIEW_DESCRIPTION"] != 'N'):?>
						...
						<!--<tr><td class="desc" <?=($collspan? 'colspan="'.$collspan.'"':"");?>><span class="desc_wrapp"><?=mb_substr($arSection[$arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]], 0, 200)?></span></td></tr>-->
						<?else:?>
