...
<div class="module-form-block-wr registraion-page">
	...
	<?}else{?>
		<?if($arResult["VALUES"]['PERSONAL_PROFESSION'] == "КП(ЮР)"){
		foreach($arResult["VALUES"] as $k=>$empty){
				$arResult["VALUES"][$k] = "";
			}
		} else {
				$arResult["VALUES"]['NAME'] = $_REQUEST["REGISTER"]['NAME'];
				$arResult["VALUES"]['LAST_NAME'] = $_REQUEST["REGISTER"]['LAST_NAME'];
	...
	<?}else{?>
		...
		<?if (count($arResult["ERRORS"]) > 0 && $arResult["VALUES"]['PERSONAL_PROFESSION'] == "КП(ФИЗ)"){
					...
					<form id="registraion-page-form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data" >
						...
						*/?>
						...
						<?foreach ($arFields as $FIELD):?>
							...
							<?}?>
										...
										<?switch ($FIELD){
											...
											<?case "PERSONAL_PROFESSION":?>
												<input size="30" type="hidden" id="input_<?=$FIELD;?>_sib" name="REGISTER[<?=$FIELD?>]" <?=($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required": "");?> value="КП(ФИЗ)" />
											<?break;?>
											<?break;
											default:?>
												<?// hide login?>
												<input size="30" id="input_<?=$FIELD;?>" <?=$FIELD == "LOGIN" ? 'type="hidden" value="1"' : 'type="text"'?> name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" />
							...
							<?}?>
						<?endforeach?>
						<input type="hidden" name="UF_LEGAL_FORM" value="1" />
						<?if($arUFields){?>
								<div class="r">
