...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.totalBlockId = parameters.totalBlockId || "";
		 ...
		 *
         *
        payByBonuses: function () {
        	if($('.enable-disable-bonuses').is(':checked')) {
        		this.result.TOTAL.ORDER_PRICE -= parseInt(this.result.TOTAL_BONUS_SUM);
        		this.result.TOTAL.ORDER_TOTAL_PRICE -= parseInt(this.result.TOTAL_BONUS_SUM);
        		this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE -= parseInt(this.result.TOTAL_BONUS_SUM);
                console.log(this.result.TOTAL.ORDER_PRICE);
                console.log(this.result.TOTAL.ORDER_TOTAL_PRICE);
                console.log(this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE);
            }else{
                this.result.TOTAL.ORDER_PRICE += parseInt(this.result.TOTAL_BONUS_SUM);
                this.result.TOTAL.ORDER_TOTAL_PRICE += parseInt(this.result.TOTAL_BONUS_SUM);
                this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE += parseInt(this.result.TOTAL_BONUS_SUM);
...
*/
		...
		getPersonTypeControl: function(node)
		{
			...
			else if (personTypesCount == 2)
			{
				...
				var isChecked = false;
				if(this.result.IS_AUTHORIZED){// && typeof(this.result.USER_PROFILES) == "object"){
					for (i in this.result.PERSON_TYPE)
					{
						if (this.result.PERSON_TYPE.hasOwnProperty(i))
						{
				...
				{	
					...
					{
						currentType = this.result.PERSON_TYPE[i];
						if(!isChecked || (isChecked && currentType.CHECKED == 'Y')) {
							label = BX.create('LABEL', {
								children: [
									BX.create('INPUT', {
		...
		getInnerPaySystem: function()
		{
			...
			var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
				...
				logotype, logoNode,subTitle, label, title, hiddenInput, htmlString, innerPsDesc, inputSumm;
			...
			inputSumm = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-updatepay'},
				children: [
					BX.create('INPUT', {
						props: {
							type: 'text',
							className: 'bx-soa-pp-company-input-text bonus-number-input',
							name: 'PAY_CURRENT_ACCOUNT_VALUE',
							value: this.result.TOTAL.PAYED_FROM_ACCOUNT?this.result.TOTAL.PAYED_FROM_ACCOUNT:''
						},
						style: {width:'50%'},
						dataset: { 
							max: this.result.CURRENT_BUDGET_MAX
						},
					}),
					BX.create('A', {
						props: {
							className: 'btn btn-default btn-md bonus-number-change',
							href: 'javascript:void(0)'
						},
						text: "Пересчитать",
						events: {
							click: BX.proxy(this.updatePaySystem, this)
						}
					})
				]
			})
			...
			htmlString =  this.params.MESS_INNER_PS_BALANCE + ' <b class="wsnw">' + this.result.CURRENT_BUDGET_FORMATED
				+ '</b><br>' + (accountOnly ? BX.message('SOA_PAY_ACCOUNT3') : '') + "<br>" + this.params.MESS_INNER_PS_BALANCE_MAX_VALUE + " " + this.result.CURRENT_BUDGET_MAX_FORMATED + "<br>";
			...
			return BX.create('DIV', {
				...
				children: [hiddenInput, subTitle, label, title, innerPsDesc, inputSumm]
			});
		},
		...
		updatePaySystem: function(event)
		{
			if (!this.orderBlockNode || !event)
				return;
			
			var target = event.target,
				bonusInput = BX.findPreviousSibling(target,{className:"bonus-number-input"}),
				maxAvailableBonus = this.result.CURRENT_BUDGET_MAX,
				maxAvailableBonusFormated = this.result.CURRENT_BUDGET_MAX_FORMATED,
				bonusInputValue, 
				isSend = false;
			...
			if(bonusInput && maxAvailableBonus){
				...
				else if(bonusInputValue > maxAvailableBonus) {
					alert("Для оплаты бонусами в этом заказе доступно " + maxAvailableBonusFormated);
				} else {
				/*	if(bonusInputValue != maxAvailableBonus)*/
						isSend = true;
				}
		...
		editTotalBlock: function()
		{
			...
			//Dance with the dead start
			/*
			if(parseInt(this.result.TOTAL_BONUS_SUM) > parseInt(this.result.BONUSES)){
                this.result.TOTAL_BONUS_SUM = this.result.BONUSES;
			}

			if(this.result.BONUSES != '0' && parseInt(this.result.BONUSES) > 0) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit('Ваши бонусы:', this.result.BONUSES + ' шт'));
				this.totalInfoBlockNode.appendChild(this.createTotalUnit('Оплатить бонусами:', this.result.TOTAL_BONUS_SUM + ' руб'));
			}

            if(this.result.TOTAL_ACCRUAL_SUM){
                this.totalInfoBlockNode.appendChild(this.createTotalUnit('Будет начислено бонусов:', this.result.TOTAL_ACCRUAL_SUM + ' шт'));
            }

            if(this.result.BONUSES != '0' && parseInt(this.result.BONUSES) > 0){
                this.totalInfoBlockNode.appendChild(
                    BX.create('DIV', {
                        props: {className: 'enable-disable-bonuses-wrapper'},
                        children: [
                            BX.create('input', {
                                props: {
									type: 'checkbox',
									name: 'enable_disable_bonuses',
                                    className: 'enable-disable-bonuses',
									'id': 'enable-disable-bonuses',
                                },
                                events: {
                                    click: BX.proxy(this.payByBonuses, this)
                                }
                            }),
                            BX.create('label', {
                                props: {
                                    for: 'enable-disable-bonuses',
                                    className: 'enable-disable-bonuses-label',
									'style': 'font-size:14px;font-weight:400;',
                                },
                                html: 'Оплатить бонусами',
                                /*events: {
			...
			this.totalBlockNode = BX(this.totalBlockId);
			if (this.totalBlockNode)
			{
				this.totalInfoBlockNode = this.totalBlockNode.querySelector('.bx-soa-cart-total');
				this.totalGhostBlockNode = this.totalBlockNode.querySelector('.bx-soa-cart-total-ghost');
