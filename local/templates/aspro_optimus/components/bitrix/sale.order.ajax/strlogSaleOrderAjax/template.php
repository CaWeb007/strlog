...
if ($useDefaultMessages || !isset($arParams['MESS_INNER_PS_BALANCE_MAX_VALUE']))
{
	$arParams['MESS_INNER_PS_BALANCE_MAX_VALUE'] = "Доступно для списания: ";
}
...
else
{
	...
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<!--	REGION BLOCK	-->
				<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active" style="display:none;">
					<div class="bx-soa-section-title-container">
						<h2 class="bx-soa-section-title col-sm-9">
	...
	if($USER->IsAuthorized())
    {
		...
		foreach($arResult['JS_DATA']['PERSON_TYPE'] as $key=>$value){
			if((int)$value['ID'] === $pId){
				$arResult['JS_DATA']['PERSON_TYPE'][$key]['CHECKED'] = "Y";
			} else {
				unset($arResult['JS_DATA']['PERSON_TYPE'][$key]['CHECKED']);
			}
		}
	}
	$signer = new Main\Security\Sign\Signer;
	$signedParams = $signer->sign(base64_encode(serialize($arParams)), 'sale.order.ajax');
	$messages = Loc::loadLanguageFile(__FILE__);if($USER->IsAdmin()) 
	?>
	<script>
