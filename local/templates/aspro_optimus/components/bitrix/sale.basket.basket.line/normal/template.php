...
<div class="basket_normal cart <?=(!$count || $isBasketPage || $_POST["ACTION"]=='top' ? ' empty_cart ' : '')?> <?=(!$count && $isBasketPage ? 'ecart' : '');?> <?=($isBasketPage || $_POST["ACTION"]=='top' ? 'bcart' : '');?>">
	<!--noindex-->
		<div class="wraps_icon_block delay <?=($delayCount ? 'fill' : '' );?>">
			<a href="<?=$arParams["PATH_TO_BASKET"];?>#delayed" class="link" <?=($delayCount ? '' : 'style="display: <!--none-->block;"' );?> title="<?=GetMessage("BASKET_DELAY_LIST");?>">Избранное</a>
			<?if($delayCount){?>
				<div class="count">
					<span>
						<div class="items">
									...
									foreach($arBasketIDs as $arItem){
										if(($arItem["CAN_BUY"] == "Y") && ($arItem["DELAY"] == "N")){
											...
											if($item["PREVIEW_PICTURE"]){
												$item["PREVIEW_PICTURE"] = CFile::ResizeImageGet((int)$item["PREVIEW_PICTURE"], array('width' => 50, 'height' => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true);
											...
											<tr class="catalog_item" product-id="<?=$arItem["ID"]?>" data-iblockid="<?=$item["IBLOCK_ID"];?>" data-offer="<?=( $item["ID"]!=$item["~ID"] ? "Y" : "N" );?>" catalog-product-id="<?=( $item["ID"]!=$item["~ID"] ? $item["~ID"] : $item["ID"] );?>">
												<td class="thumb-cell">
													<a href="<?=$arItem["DETAIL_PAGE_URL"]?>">
														<!--<img src="<?/*=$arItem["PICTURE_SRC"]?>" alt="<?=($item["PREVIEW_PICTURE"]["alt"] ? $item["PREVIEW_PICTURE"]["alt"] : $arItem["NAME"]);?>" title="<?=($item["PREVIEW_PICTURE"]["title"] ? $item["PREVIEW_PICTURE"]["title"] : $arItem["NAME"]);*/?>" />-->
														<?if($item["PREVIEW_PICTURE"]):?>
														...
														<?elseif($item["DETAIL_PICTURE"]):?>
                                                            <img src="<?=$item["DETAIL_PICTURE"]["src"]?>" alt="<?=($item["DETAIL_PICTURE"]["alt"] ? $item["DETAIL_PICTURE"]["alt"] : $arItem["NAME"]);?>" title="<?=($item["DETAIL_PICTURE"]["title"] ? $item["DETAIL_PICTURE"]["title"] : $arItem["NAME"]);?>" />
                                                        <?else:?>
