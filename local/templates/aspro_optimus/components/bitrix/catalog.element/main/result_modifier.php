...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if(isset($arResult["PROPERTIES"]["CML2_TRAITS"])){
		foreach($arResult["PROPERTIES"]["CML2_TRAITS"]["DESCRIPTION"] as $dk=>$dv){
			if($dv == "Ценовая группа"){
				$isPriceC = isIssetPG($arResult["PROPERTIES"]["CML2_TRAITS"]["VALUE"][$dk]);break;
			}
		}
	}

	if($isPriceC) {
		if(isset($arResult['PRICES']['ТО'])){
			unset($arResult['PRICES']['ТО']);
			foreach($arResult['OFFERS'] as $kOffer=>$arROffer){
				unset($arResult['OFFERS'][$kOffer]['PRICES']['ТО']);
			}
		}
	} else {
		if(isset($arResult['PRICES']['С'])){
			unset($arResult['PRICES']['С']);
			foreach($arResult['OFFERS'] as $kOffer=>$arROffer){
				unset($arResult['OFFERS'][$kOffer]['PRICES']['С']);
			}
		}
	}
	/* end PRICE 'TO' */
	#varDump($arResult['OFFERS']);
...
/* OLD PRICE */
if(isset($arResult["PROPERTIES"]["OLDPRICE_TO"]) || isset($arResult["PROPERTIES"]["OLDPRICE_SO"]) || isset($arResult["PROPERTIES"]["OLDPRICE_KP"])){
	$arPRICES_ID = ['ТО' => 'TO','СО' => 'SO','КП' => 'KP'];
	if((float)$arResult["PROPERTIES"]["OLDPRICE_TO"] > 0 || (float)$arResult["PROPERTIES"]["OLDPRICE_SO"] > 0 || (float)$arResult["PROPERTIES"]["OLDPRICE_KP"] > 0) {
		foreach($arResult['PRICES'] as $PKEY => $PRICE){
			if(isset($arPRICES_ID[$PKEY]) && (float)$arResult["PROPERTIES"]["OLDPRICE_" . $arPRICES_ID[$PKEY]]['VALUE'] > 0) {
				$oldPrice = (float)$arResult["PROPERTIES"]["OLDPRICE_" . $arPRICES_ID[$PKEY]]['VALUE'];
				if((float)$PRICE['DISCOUNT_VALUE'] > 0 && (float)$PRICE['DISCOUNT_VALUE'] < $oldPrice){
					$arResult['PRICES'][$PKEY]['VALUE'] = $oldPrice;
					$arResult['PRICES'][$PKEY]['PRINT_VALUE'] = $oldPrice . " руб";
				}elseif((float)$PRICE['VALUE'] > 0 && (float)$PRICE['VALUE'] < $oldPrice){
					$arResult['PRICES'][$PKEY]['DISCOUNT_VALUE'] = $arResult['PRICES'][$PKEY]['VALUE'];
					$arResult['PRICES'][$PKEY]['PRINT_DISCOUNT_VALUE'] = $arResult['PRICES'][$PKEY]['VALUE'] . " руб";
					$arResult['PRICES'][$PKEY]['VALUE'] = $oldPrice;
					$arResult['PRICES'][$PKEY]['PRINT_VALUE'] = $oldPrice . " руб";
				}
			}
...
if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arBrand = COptimusCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => COptimusCache::GetIBlockCacheTag($arResult["PROPERTIES"]["BREND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]));
	if($arBrand){
...
<?COptimus::AddMeta(
	array(
		'og:description' => $arResult['DETAIL_TEXT'] ? $arResult['DETAIL_TEXT'] : $arResult['PREVIEW_TEXT'],
