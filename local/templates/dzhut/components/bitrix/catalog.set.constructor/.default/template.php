...
<div id="bx-set-const-<?=$curJsId?>" class="bx-set-constructor container-fluid <?=$templateData['TEMPLATE_CLASS'];?>">
	<div class="row">
		<div class="col-xs-12">
			<strong class="bx-modal-small-title" style="font-size:24px;color:#f04e23;">В наборе ДЕШЕВЛЕ! &dArr;</strong>
		</div>
	</div>
	<div class="row">
		<!--div class="col-sm-3">
					...
					<tbody data-role="set-items">
						<tr>
							<td class="bx-added-item-table-cell-img">
								<?if ($arResult["ELEMENT"]["DETAIL_PICTURE"]["src"]):?>
									<img src="<?=$arResult["ELEMENT"]["DETAIL_PICTURE"]["src"]?>" class="img-responsive" alt="">
								<?else:?>
									<img src="<?=$this->GetFolder().'/images/no_foto.png'?>" class="img-responsive" alt="">
								<?endif?>
							</td>
							<td class="bx-added-item-table-cell-itemname">
								<a class="tdn" href="#"><?=$arResult["ELEMENT"]["NAME"]?></a>
							</td>
							<td class="bx-added-item-table-cell-price">
								<span class="bx-added-item-new-price"><?=$arResult["ELEMENT"]["PRICE_PRINT_DISCOUNT_VALUE"]?> * <?=$arResult["ELEMENT"]["BASKET_QUANTITY"];?> <?=$arResult["ELEMENT"]["MEASURE"]["SYMBOL_RUS"];?></span>
								<?if (!($arResult["ELEMENT"]["PRICE_VALUE"] == $arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"])):?>
									<br><span class="bx-added-item-old-price"><?=$arResult["ELEMENT"]["PRICE_PRINT_VALUE"]?></span>
								<?endif?>
							</td>
							<td class="bx-added-item-table-cell-del"></td>
						</tr>
					<?foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):?>
						...
						>
							...
							<td class="bx-added-item-table-cell-price">
								...
								<br><span class="bx-added-item-old-price"><b><?=$arItem["PRICE_PRINT_VALUE"]?></b></span>
								<?endif?>
							</td>
		...
		<div class="col-sm-5 col-sm-offset-3">
			<table class="bx-constructor-result-table">
				<tr style="display: <?=($arResult['SHOW_DEFAULT_SET_DISCOUNT'] ? 'table-row' : 'none'); ?>;">
					<td class="bx-constructor-result-table-title"></td>
					<td class="bx-constructor-result-table-value">
						<strong data-role="set-price" style="font-size: 30px !important;"><?=$arResult["SET_ITEMS"]["PRICE"]?></strong> <br> <strong data-role="set-old-price"> <s><?=$arResult["SET_ITEMS"]["OLD_PRICE"]?></s></strong>
					</td>
				</tr>
				<!--<tr>
				...
				<tr style="display: <?=($arResult['SHOW_DEFAULT_SET_DISCOUNT'] ? 'table-row' : 'none'); ?>;">
					<td class="bx-constructor-result-table-title"></td>
					<td class="bx-constructor-result-table-value">
						<?=GetMessage("CATALOG_SET_ECONOMY_PRICE")?>: <strong style="color:#86ad00;" data-role="set-diff-price"><?=$arResult["SET_ITEMS"]["PRICE_DISCOUNT_DIFFERENCE"]?></strong>
					</td>
				</tr>
			</table>
		</div>
