...
foreach ($arResult['ITEMS'] as $key => $arItem){
	$catPrice = $arItem["CATALOG_PRICE_1"];
	$catPercent = $arItem["PROPERTIES"]["_1"]["VALUE"];
	$totalDiscount = IntVal($catPrice) + ((IntVal($catPercent) / IntVal(100)) * IntVal($catPrice));
	echo $totalDiscount;
	
	//==== Get Complekt
$arSets = CCatalogProductSet::getAllSetsByProduct($arItem['ID'], CCatalogProductSet::TYPE_SET);
	...
	<div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		...
		<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>')" title="<? echo $imgTitle; ?>">
			...
			{
			?>
					<div id="<? echo $arItemIDs['DSC_PERC']; ?>" class="bx_stick_disc right bottom" style="display:<? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;"></div>
			<?
			}
			if ($arItem['LABEL'])
			{
	...
	<div class="bx_catalog_item_price">
		...
		<?
			...
			unset($minPrice);
			?>
			<span style="font-size:12px;color:#999999;">/ <?=$arItem['PROPERTIES']['CML2_BASE_UNIT']['VALUE'];?></span><br />
			<?if(!empty($arItem["PROPERTIES"]["_1"]["VALUE"]) && $arItem["PROPERTIES"]["_1"]["VALUE"] != ''):?>
			<span class="old-price">
				<?echo $totalDiscount;?>
				<ruble><span class="text"></span></ruble>
			</span>
			<?endif;?>
		</div>
	</div>
		<div class="product-bonus">
			<a href="">Бонус <span class="orange"><?=$arItem['PROPERTIES']['BONUS']['VALUE'][0]?></span></a>
				</div>
				<div class="desc-bonus">
					<p class="orange">Копите бонусы и оплачивайте
		ими ваши покупки!</p>
				<p>1 бонус = 1 рубль</p>
				<a class="orange" href="http://strlogclub.ru/" target="_blank">Узнать больше</a>
				</div>
	<?
	...
	if (false)
	{
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium hello" href="<?echo $arElement["BUY_URL"]?>" rel="nofollow"><? echo GetMessage('CT_BCS_TPL_MESS_BTN_BUY')?></a>
		</div>
			<?
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		...
		<div class="bx_catalog_item_controls no_touch">
			<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo wrap-buy">
				
				<?if(in_array($arItem['ID'],$CheckBasket)):?>
					<a class="bx_bt_button bx_medium success-btn qqq" href="/personal/cart/" rel="nofollow"><?echo GetMessage("ADD_TO_BASKET_IN")?></a>
				<?else:?>
				<?$exp = explode("?",$arItem["BUY_URL"]);if(isset($exp[1]))$arItem["BUY_URL"]="/catalog/brusok_1/?".$exp[1]?>
				<a class="bx_bt_button bx_medium" href="<?echo $arItem["BUY_URL"]?>" rel="nofollow"><?echo GetMessage("CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET")?></a>
				<?endif;?>
			</div>
...
(function(){
	jQuery(document).ready(function(){
		...
		$(".bx_bt_button.bx_medium").unbind("click"); // удалим 2 лишних обработчика и установим один, иначе срабатывают 3 обработчикка события и добавляют в корзину сразу 3 элемента поочередно 

		$(".bx_bt_button.bx_medium").not('.no-ajax').on('click',function(e){
