...
if (!empty($arResult['ITEMS']))
{
	...
	$arAttachProductID = array();
	// Запишим все ID прикрепленных товаров
	foreach($arResult['ITEMS'] as $CountProd){
		$arAttachProductID[] = 	$CountProd['ID'];
	}
	// Делаем запрос по остаткам на складах 
	$GetStore = CCatalogStoreProduct::GetList(array(),array("PRODUCT_ID"=>$arAttachProductID),false,false,array("AMOUNT","PRODUCT_ID"));
	// Сохраним результат выборки в массив $arResultStore
	$arResultStore = array();
	
	while($GetStoreResult = $GetStore->GetNext()){
			array_push($arResultStore,array("PRODUCT_ID"=>$GetStoreResult["PRODUCT_ID"],"AMOUNT"=>$GetStoreResult["AMOUNT"])); 
	}
	$ProductID = "";
	$Amount = "";
	$ArSote = array();
...
/* 	foreach($arResultStore as $TestVal){
		

		if(empty($ProductID) && empty($Amount)){

			$ProductID = $TestVal['PRODUCT_ID'];
			$Amount = $TestVal['AMOUNT'];	
			
		}else if($ProductID == $TestVal['PRODUCT_ID']){
				echo $Amount += $TestVal['AMOUNT'];
				
		}else{
			array_push($ArSote,array("PRODUCT_ID" => $ProductID,"AMOUNT" => $Amount);
			$ProductID = $TestVal['PRODUCT_ID'];
			$Amount = $TestVal['AMOUNT'];
		} 
