...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
				<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium <?if($arItem['INSIDE_BASKET'] == 'Y'): echo 'disable';endif;?>" href="javascript:void(0)" rel="nofollow"><?
				if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
				{
					//echo ('' != $arParams['MESS_BTN_BUY']  $arItem['INSIDE_BASKET'] == 'Y' ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
					
					if($arItem['INSIDE_BASKET'] == 'Y'){
						echo etMessage('CT_BCS_TPL_MESS_BTN_INSIDE_BASKET');
					}
					else if('' != $arParams['MESS_BTN_BUY']){
						echo $arParams['MESS_BTN_BUY'];
					}
					else{
						echo GetMessage('CT_BCS_TPL_MESS_BTN_BUY');
					}
				}
				else
				{
				 //echo ('' != $arParams['MESS_BTN_ADD_TO_BASKET'] ? $arParams['MESS_BTN_ADD_TO_BASKET'] : GetMessage('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET'));
					if($arItem['INSIDE_BASKET'] == 'Y'){
						echo GetMessage('CT_BCS_TPL_MESS_BTN_INSIDE_BASKET');
					}
					else if('' != $arParams['MESS_BTN_ADD_TO_BASKET']){
						
						echo $arParams['MESS_BTN_ADD_TO_BASKET'];
						
					}
					else{
						echo GetMessage('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET');
					}
				}
				?></a>
			</div>
