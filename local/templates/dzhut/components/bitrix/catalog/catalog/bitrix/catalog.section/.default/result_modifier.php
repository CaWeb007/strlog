...
if (!empty($arResult['ITEMS']))
{
	...
	// echo "<pre>";
		...
		$arBasket = CSaleBasket::GetList(array(),array("FUSER_ID"=>CSaleBasket::GetBasketUserID(),"ORDER_ID"=>NULL),false,false,array("PRODUCT_ID"));
		while($arBasketRow = $arBasket->Fetch()){
			// echo $arBasketRow['PRODUCT_ID'];
			foreach($arResult['ITEMS'] as $key => $Offers){
				foreach($Offers['OFFERS'] as $OffersID){
					if($arBasketRow['PRODUCT_ID'] == $OffersID['ID']){
						 $arResult['ITEMS'][$key]['INSIDE_BASKET'] = "Y";
					} 
				}
			}
		}
