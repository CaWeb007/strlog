...
?>
	...
	<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="item_current_price"><span itemprop="price"><? echo $minPrice['PRINT_DISCOUNT_VALUE']; ?></span></div>
		...
		<div class="bx-tab description active">
			...
			<?if(!empty($arResult['PROPERTIES']['NO_EMPTY_HR'])){?>
				...
				?>
					...
					<?
						$NoView = array('ATT_YOUTUBE','BEST_PRICE','RAZRESHENNYY_OPLATY_BONUSAMI','_POROGA_NACHISLENIYA_BONUSOV','NAIMENOVANIE_DLYA_SAYTA','CML2_BAR_CODE','CML2_ARTICLE','CML2_ATTRIBUTES','CML2_TRAITS','CML2_BASE_UNIT','CML2_TAXES','CML2_MANUFACTURER','BLOG_POST_ID','BLOG_COMMENTS_CNT','BONUS','ON_DESCOUNT','AMOUNT_OF_DISCOUNT','ATT_INSTRUCTIONS','BREND_LOGO','BREND','MORE_PHOTO','FILES','HIT','BEST','NEW','MAIN','NALICHIE','NEKONDITSIYA','PROIZVODITEL_1');
...
<span style="font-size:12px;color:#c7c6c6;"><?
echo (
	isset($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]) && $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] != ''
	? $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]
	: $arResult["NAME"]
...
echo (
	...
	? $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]
	: $arResult["NAME"]
); ?> в Иркутске по доступной цене, недорого, дешево. Продажа
<? $db_old_groups = CIBlockElement::GetElementGroups($arResult['ID'], false);
while($ar_group = $db_old_groups->Fetch()) {
echo $ar_group["NAME"];
}
?>. В нашем интернет магазине представлена лучшая цена для покупателей. </span>
