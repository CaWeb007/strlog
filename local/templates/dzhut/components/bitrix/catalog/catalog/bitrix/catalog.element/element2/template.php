...
<div class="bx_item_title">
	<h1><?echo (isset($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]) && $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] != ''
			? trim($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"])
			: trim($arResult["NAME"])); ?></h1>
</div>
	<div class="find-cheap-onload"></div>
	<div style="display: none" class="popup-find-cheap-wrapper">
		<div class="popup-find-cheap-close">&times;</div>
		<div class="popup-find-cheap-title">Нашли дешевле? Мы снизим цену!</div>
		<form id="form-find-cheap" action="javascript:void(null);">
			<input class="find-cheap-data" name="find_cheap_name" type="text" placeholder="Имя" required />
			<input class="find-cheap-data" name="find_cheap_phone" type="text" placeholder="Телефон" required />
			<input class="find-cheap-data" name="find_cheap_email" type="text" placeholder="E-mail" />
			<input class="find-cheap-data" name="find_cheap_link" type="text" placeholder="Ссылка на товар или название магазина" required />
			<input class="find-cheap-data" name="find_cheap_price" type="text" placeholder="Цена" required />
			<input class="find-cheap-send" value="Отправить" type="submit" />
		</form>
		<!--agreePersonalButton-->
		<label id="labelFindCheapAgreePersonalButton" for="findCheapAgreePersonalCheckbox">
			<input id="findCheapAgreePersonalCheckbox" type="checkbox" value="" checked />
			<span id="findCheapAgreePersonalText">Я согласен на <a href="/agreement/" target="_blank">обработку персональных данных</a> и с <a href="/find-cheap-page/" target="_blank">правилами участия в акции</a></span>
		</label>
		<!--/agreePersonalButton-->
		<div class="popup-find-cheap-thanks">Спасибо. Наши менеджеры свяжутся с вами в ближайшее время</div>
	</div>
	<div class="notRegOrAuth-popup-find-cheap-wrapper">
		<div class="notRegOrAuth-popup-find-cheap-close">&times;</div>
		<div class="notRegOrAuth-popup-find-cheap-title">Нашли дешевле? Мы снизим цену!</div>
		<div class="notRegOrAuth-popup-find-cheap-desc">Функция доступна только зарегистрированным пользователям</div>
		<div class="notRegOrAuth-reg-auth-cancel">
			<div class="notRegOrAuth-reg">Регистрация</div>
			<div class="notRegOrAuth-auth">Войти</div>
		</div>
		<span id="findCheapAgreePersonalText">
			<a href="/find-cheap-page/" target="_blank">Правила участия в акции</a>
		</span>
	</div>
<?
}
...
<div class="bx_item_slider" id="<? echo $arItemIDs['BIG_SLIDER_ID']; ?>">
	<ul class="element-actions-wrapper" style="display:none;">
		<li class="element-action element-action-3">-3%</li>
		<li class="element-action element-action-20">-20%</li>
		<li class="element-action element-action-35">-35%</li>
		<li class="element-action element-action-new">Новинка</li>
		<li class="element-action element-action-hit">Хит продаж</li>
		<li class="element-action element-action-best">Лучшая цена</li>
	</ul>
<!---->
<div id="db-items">
<div class="bx_bigimages" id="<? echo $arItemIDs['BIG_IMG_CONT_ID']; ?>">

	<div  class="bx_bigimages_imgcontainer" <?if (empty($arFirstPhoto['SRC'])) echo "style=\"background: url('/bitrix/templates/universal/image/no_photo.png') center center;\"";?>>
		<span class="bx_bigimages_aligner" <?if (empty($arFirstPhoto['SRC'])) echo "style=\"background: url('/bitrix/templates/universal/image/no_photo.png') center center;\"";?>>
		<img id="<? echo $arItemIDs['PICT']; ?>" src="<?=$arFirstPhoto['SRC']?>"/>
			...
			<?}?>
			<?/*if(!empty($arResult['PROPERTIES']['ACTION_1']['VALUE'])){?>
				<div class="best_price action">
					Акция
				</div>
			<?}*/?>
			<?if(!empty($arResult['PROPERTIES']['ACTION_1']['VALUE'])){?>
				<?if($arResult['PROPERTIES']['ACTION_1']['VALUE'] == 20):?>
				<div class="best_price percent">
					-20%
				</div>
				<?elseif($arResult['PROPERTIES']['ACTION_1']['VALUE'] == "распродажа"):?>
				...
				<?endif;?>
			<?}?>
			<?/*if(!empty($arResult['PROPERTIES']['ACTION']['VALUE'])){?>
				<div class="best_price percent">
					Акция
				</div>
			<?}*/?>
...
<?
		...
		$sliderImg = CFile::ResizeImageGet($arOnePhoto['ID'],array("width"=>88,"height"=>88),BX_RESIZE_IMAGE_PROPORTIONAL);
		 if($iter<4){
			 if(empty($arOnePhoto['SRC'])){
				 $arOnePhoto['SRC'] = SITE_TEMPLATE_PATH . "/image/no_photo.png";
			 }

					if(empty($sliderImg['SRC'])){
						$sliderImg['SRC'] = SITE_TEMPLATE_PATH . "/image/no_photo.png";
					}

?>

		<div class="wrap-img-slide" <?if(!empty($sliderImg['src'])):?> style="background: transparent;" <?endif;?>>
...
<script>
$("#<? echo $arItemIDs['PICT']; ?>").elevateZoom({
 gallery:'gal1',
 cursor: 'pointer',
 galleryActiveClass: 'active',
 borderSize : 0
 });
 $("#<? echo $arItemIDs['PICT']; ?>, .thumb").on("click", function(e) {
	var ez = $('#<? echo $arItemIDs['PICT']; ?>').data('elevateZoom');
	$.fancybox(ez.getGalleryList()); return false;
});
</script>

</div><br>
		<span style="font-size:12px;color:red">Внимание! Изображения товаров на фотографиях, представленных в каталоге на сайте, могут отличаться от оригиналов - уточняйте у менеджеров!</span>
</div>
...
?>
	...
	<?if(!empty($arResult['PROPERTIES']['TSENA_ZA_M2']['VALUE'])):?>
	<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="item_current_price"><span itemprop="price"><? echo $arResult['PROPERTIES']['TSENA_ZA_M2']['VALUE']; ?><ruble><span class="text"><meta itemprop="priceCurrency" content="RUB" /></span></ruble><span style="font-size:12px;color:#999999;"> / м<sup>2</sup></span></div>
	<?elseif(!empty($arResult['PROPERTIES']['TSENA_ZA_KG']['VALUE'])):?>
	<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="item_current_price"><span itemprop="price"><? echo $arResult['PROPERTIES']['TSENA_ZA_KG']['VALUE']; ?><ruble><span class="text"><meta itemprop="priceCurrency" content="RUB" /></span></ruble><span style="font-size:12px;color:#999999;"> / кг</span></div>
<?else:?>
	<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="item_current_price"><span itemprop="price"><? echo $minPrice['PRINT_DISCOUNT_VALUE']; ?><span style="font-size:12px;color:#999999;">/ <?=$arResult['PROPERTIES']['CML2_BASE_UNIT']['VALUE'];?></span></div>
	<?endif;?>
	<?if($arResult['PROPERTIES']['PROIZVODITEL_1']['VALUE'] == 'Docke'):?>
		<div style="text-align:center;">
			<span class="old-price" style="text-align: center;font-size:1.2rem;position:relative;display:inline-block;margin-top:0.5rem;opacity:0.5;font-family:'BlissPro-Regular';font-weight:normal;">
			<?$dockeOldPrice = IntVal($minPrice['PRINT_DISCOUNT_VALUE']) + (IntVal($minPrice['PRINT_DISCOUNT_VALUE']) * 0.3);?>
			<?=$dockeOldPrice;?>
			<ruble>
	...
	<?endif;?>
		<?/*if($arResult['PROPERTIES']['ACTION_1']['VALUE'] == 'распродажа'):?>
			<span class="item_old_price" style="display:block;text-align:center;">
				<?if(!empty($arResult['PROPERTIES']['TSENA_ZA_M2']['VALUE'])):?>
					<? echo round($arResult['PROPERTIES']['TSENA_ZA_M2']['VALUE']+$arResult['PROPERTIES']['TSENA_ZA_M2']['VALUE']/100*35); ?>
				<?else:?>
					<? echo round($minPrice['PRINT_DISCOUNT_VALUE']+$minPrice['PRINT_DISCOUNT_VALUE']/100*35); ?>
				<?endif;?>
				<ruble><span class="text"><meta itemprop="priceCurrency" content="RUB"></span></ruble>
			</span>
		<?endif;*/?>

		<div class="product-bonus hi">
...
<?
			...
			#	if(trim($arOneValue['NAME']) == "-") continue;
?>
...
?>
	...
	<div class="item_buttons vam">
		<span class="item_buttons_counter_block hello">
			<a href="javascript:void(0)" class="bx_bt_button_type_2 bx_small bx_fwb down" id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>">-</a>
			<input id="<? echo $arItemIDs['QUANTITY']; ?>" type="number" class="tac transparent_input" value="<? echo (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])
					? 1
...
<!--Изначально функция была эта-->
<?/*$APPLICATION->ShowViewContent("store_amount");*/?>
<!--/Изначально функция была эта-->

<!--Заменена на эту-->
<?$quant = 1 * $arResult['CATALOG_QUANTITY'];?>
<div class="amount_store" style="margin: 7px auto 0;padding:0;font-size: 16px;line-height:1;">
<?if($quant > 0):?>
	Есть в наличии
<?else:?>
	Под заказ
...
<?else:?>
	...
	<?else:?>
		<a href="javascript:void(0);" class="bx_big bx_bt_button bx_cart" id="<? echo $arItemIDs['ADD_BASKET_LINK']; ?>"><span></span><? echo $buyBtnMessage; ?></a><!--ADD_BASKET_LINK-->
	<?endif;?>
<?endif;?>
	...
	<div class="row-desc">
		...
		<div class="wrap-brend"><img src="<?=$FileBrend['SRC']?>"></div>
				...
				<div class="row-desc-text">
					...
					<?}?>
<?/*
					<div class="find-cheap-wrapper">
						<?if($USER->IsAuthorized()):?>
						<div class="show-find-cheap-popup">Нашли дешевле? Снизим цену!</div>
						<?else:?>
						<div class="go-authorize-find-cheap-popup">Нашли дешевле? Снизим цену!</div>
						<?endif;?>
					</div>
*/?>
...
<div class="bx_md">
<div class="item_info_section dwrty">
<?
if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
...
<div class="bx-tabs">
		<div class="bx-tab description active">
			<?if(!empty($arResult['DETAIL_TEXT'])){?>
				<h2>Продажа <?=$arResult["NAME"]?> в Иркутске по низкой цене</h2>
					<p class="text_description"><?=$arResult['DETAIL_TEXT']?></p>
			<?}?>
			<?if(!empty($arResult['PROPERTIES']['NO_EMPTY_HR'])){?>
				<h3>Характеристики</h3>
					<table class="property-table">
					<?
						$NoView = array('BRAND','ACTION_1','SAP','ISOVER_ID','VYVOD','ATT_YOUTUBE','BEST_PRICE','RAZRESHENNYY_OPLATY_BONUSAMI','_POROGA_NACHISLENIYA_BONUSOV','NAIMENOVANIE_DLYA_SAYTA','CML2_BAR_CODE','CML2_ARTICLE','CML2_ATTRIBUTES','CML2_TRAITS','CML2_BASE_UNIT','CML2_TAXES','CML2_MANUFACTURER','BLOG_POST_ID','BLOG_COMMENTS_CNT','BONUS','ON_DESCOUNT','AMOUNT_OF_DISCOUNT','ATT_INSTRUCTIONS','BREND_LOGO','BREND','MORE_PHOTO','FILES','HIT','BEST','NEW','MAIN','NALICHIE','NEKONDITSIYA','PROIZVODITEL_1','NALICHIE','NELZYA_OPLACHIVAT_BONUSAMI','NE_NACHISLYAT_BONUSY','FLAG');
			...
			<?}?>
<br>
			<!--<i style="font-size:12px;">*** На данной странице сайта вы можете купить <?
echo (
...
// Work round
	$(".blog-add-comment a b").text("Добавить отзыв");
		var commentAuthor = $('div.blog-comment-field-author input');
		var commentEmail = $('div.blog-comment-field-email input');
		var commentBody = $(".lha-iframe").contents().find('body').text();

		$('div.blog-comment-field-author').find('input').attr('placeholder', 'Ваше имя').parent().parent().find('.blog-comment-field-text').remove();
		$('div.blog-comment-field-email').find('input').attr('placeholder', 'Ваш e-mail').parent().parent().find('.blog-comment-field-text').remove();


		$('.blog-comment-buttons input').removeAttr('onclick');
		$('.blog-comment-buttons input').on('click',function(e){



			if ($(".lha-iframe").contents().find('body').text().length == 0) {
				$('.bxlhe-frame').addClass('error');
			} else {
				$('.bxlhe-frame').removeClass('error');
			}

			if (commentAuthor.val() != undefined) {
				if (commentAuthor.val().length == 0) {
					commentAuthor.addClass('error');
				} else {
					commentAuthor.removeClass('error');
				}
			}


			if (commentEmail.val() != undefined) {
				var emailVal = validateEmail(commentEmail);
			}


			if ( $(".lha-iframe").contents().find('body').text().length != 0 && ( commentAuthor.length != 0 || commentAuthor.val() == undefined ) && ( emailVal || commentEmail.val() == undefined ) ) {
				submitComment();
				console.log('yep');
			} else {
				console.log('nope');
