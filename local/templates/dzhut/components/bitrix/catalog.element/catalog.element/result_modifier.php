...
//==== Добавляю фото 
foreach($arResult['MORE_PHOTO'] as $key => $IDImg){
	
		$Resize = CFile::ResizeImageGet($IDImg['ID'],array("width"=>600,"height"=>600),BX_RESIZE_IMAGE_PROPORTIONAL,true);
		
		$arResult['MORE_PHOTO'][$key]['SRC'] = $Resize['src'];
		$arResult['MORE_PHOTO'][$key]['WIDTH'] = $Resize['width'];
		$arResult['MORE_PHOTO'][$key]['HEIGHT'] = $Resize['height'];
		
}
//==== SEO 
foreach($arResult["IPROPERTY_VALUES"] as $key => $value){
	$PriceNoFloat = explode(".", $arResult["PRICES"]["BASE"]["VALUE"]);
    $arResult["IPROPERTY_VALUES"][$key] = str_replace("{component.price}",$PriceNoFloat[0], $arResult["IPROPERTY_VALUES"][$key]);
}
	
//=== Не пустые характеристики
$NoView = array('BEST_PRICE','RAZRESHENNYY_OPLATY_BONUSAMI','_POROGA_NACHISLENIYA_BONUSOV','NAIMENOVANIE_DLYA_SAYTA','CML2_BAR_CODE','CML2_ARTICLE','CML2_ATTRIBUTES','CML2_TRAITS','CML2_BASE_UNIT','CML2_TAXES','CML2_MANUFACTURER','BLOG_POST_ID','BLOG_COMMENTS_CNT','BONUS','ON_DESCOUNT','AMOUNT_OF_DISCOUNT','ATT_INSTRUCTIONS','BREND_LOGO','BREND','MORE_PHOTO','FILES');
foreach($arResult['PROPERTIES'] as $key => $value){
		if(!in_array($key,$NoView)){
			if(!empty($value['VALUE'])){			
			$arResult['PROPERTIES']['NO_EMPTY_HR'] = 'Y';
		}		
	}		
}

(int)$price = round($arResult['PRICES']['BASE']['VALUE']);
$old_pr = ($price*100)/(100 - 50);
$arResult['PR_OLD_PRICE'] = round($old_pr);
/*(int)$percent = $arResult['PROPERTIES']['_1']['VALUE'] * $price / 100;
$arResult['PR_OLD_PRICE'] = $percent + $price;*/
/* foreach($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $IDImg){
		$Resize = CFile::ResizeImageGet($IDImg,array("width"=>600,"height"=>600),BX_RESIZE_IMAGE_PROPORTIONAL,true);
		$arResult['MORE_PHOTO'][] = array("ID"=>$IDImg,"SRC"=>$Resize['src'],"WIDTH"=>$Resize['width'],"HEIGHT"=>$Resize['height']);
} */ 
//edebug($arResult);
