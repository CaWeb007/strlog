...
?>
<form id="check-bonus" style="display:none;">
	<input class="user-phone" type="hidden" name="user_phone" value="<?=$userPhone;?>" />
</form>
<!--/USER FIELDS-->
...
<a title="Контакты" href="/gde-kupit/">Контакты</a>
...
<form action="<?=$arResult["AUTH_URL"]?>">
		...
		<div class="auth-el">
			<a href="/personal/bonusy/" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
				<?=GetMessage("YOU_HAVE_BONUS")?>
				<span class="personal-bonus"></span> 
				<?/*if(empty($arResult['USER_BONUS'])) $arResult['USER_BONUS'] = 0; echo  $arResult['USER_BONUS'];*/?> 
				<?echo declension($arResult['USER_BONUS'],GetMessage("YOU_HAVE_BONUS_ONE"),GetMessage("YOU_HAVE_BONUS_ODD"),GetMessage("YOU_HAVE_BONUS_EVEN"))?>
			</a>
			<div class="dropdown-menu-right">
				<p class="bonus-depoz-text">
					<?=GetMessage("YOU_HAVE_DEPOSIT")?>
					<span class="orange"><span class="personal-bonus"></span><?/*=$arResult['USER_BONUS'] . " ". declension($arResult['USER_BONUS'],GetMessage("YOU_HAVE_BONUS_ONE"),GetMessage("YOU_HAVE_BONUS_ODD"),GetMessage("YOU_HAVE_BONUS_EVEN"))*/?> бонусов</span>
				</p>
				<div class='orange-link'>
					<a target="_blank" href="http://strlogclub.ru/about/"><?=GetMessage("DEPOSIT_MORE")?></a>
				</div>
			</div>
		</div>
		<!--//BONUS DROP-->
