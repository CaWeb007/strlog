...
/* mmit - 03.08.2016 - Andrey Vlasov - start */

	// заполняем select выбора даты рождения значениями
	writeDate();

/* не даем отправить форму, если телефон не подтвержден */
	$('form[name="form1"]').on("submit", function(event) {
		if ($('form[name="form1"] #CHECKPHONE').val() == "N") {
			event.preventDefault();
			$('form[name="form1"] #error_confirm').css('display','block');
			setTimeout(function() {
				$('form[name="form1"] #error_confirm').css('display','none');
			}, 2500);
		}
	});

/* если изменяем номер телефона, то выводим кнопку его подтверждения */
	$('form[name="form1"] input[name="PERSONAL_PHONE"]').on('keyup', function(event) {
		var current_tel = $(this).val();
		var default_tel = $(this).data("tel");
		if (current_tel != default_tel) {
			$('form[name="form1"] .tl_code').removeClass("inactive");
			$('form[name="form1"] .item .confirm_tel').addClass("inactive");
			$('form[name="form1"] #CHECKPHONE').val("N");
		} else {
			$('form[name="form1"] .tl_code').addClass("inactive");
			$('form[name="form1"] .item .confirm_tel').removeClass("inactive");
			$('form[name="form1"] #CHECKPHONE').val("Y");
		}
	});
	...
	$("#checkTelForm1").on('click',function(e){
		...
		var $orderPhone = validateTel($('form[name="form1"] input[name="PERSONAL_PHONE"]'));
			...
			userID = $('form[name="form1"] input[name="ID"]').val();
	...
	$("#sendCodeForm1").on('click',function(e){
		...
		if(!$orderPhone){
			...
			var userPhone = $('form[name="form1"] input[name="PERSONAL_PHONE"]').val();
	...
	$("#re-confirm-top-form1").on('click',function(e){
		e.preventDefault();
			...
			var $orderPhone = validateTel($('form[name="form1"] input[name="PERSONAL_PHONE"]'));
			var userID = $('form[name="form1"] input[name="ID"]').val();
				if($orderPhone){
					var Reconfirm = "Y";
					userPhone = $('form[name="form1"] input[name="PERSONAL_PHONE"]').val();
					$.ajax({
						...
						data:{Reconfirm:Reconfirm,userPhone:userPhone,userId:userID},
						success:function(data){
							
							if(data.user == "Y") {
								$(".topltip_error_top").show();
								$(".desc_confirm_top").hide();
								$('form[name="form1"] input[name="PERSONAL_PHONE"]').addClass('error').focus(function() {
									$(this).off("focus").removeClass('error');
								});
								setTimeout(function() {
									$(".topltip_error_top").hide();
								},3000);
							};
