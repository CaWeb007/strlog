		...
		if (!empty($arSource))
		{
			...
			<div>
				...
				foreach ($arSource as $arProperties)
				{
					...
					<?if(intval(intval($arProperties["ID"])) == 6) echo "style='display:none;'"; ?>>
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<div class="bx_block r1x3 pt8">
								...
								<input <?if($arProperties["FIELD_NAME"]=="ORDER_PROP_9"):?>checked="checked"<?endif?> type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" value="Y" <?if($USER->IsAuthorized()):?> checked="checked" <?endif;?>>
						...
						{
							?>
							<div class="bx_block r3x1 phone" >
								
							<?//====Дополнительные свойства Фамилия, Имя, Отчество ====//?>
								
								<?if($arProperties["FIELD_NAME"] == "ORDER_PROP_10"):?>								
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="Фамилия"/>
									<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
										<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
									<?endif?>
								<?endif;?>
								...
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="Имя"/>
									<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
										<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
									<?endif?>
								<?endif;?>
								
								<?if($arProperties["FIELD_NAME"] == "ORDER_PROP_11"):?>								
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="Отчество"/>
									<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
										<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
									<?endif?>
								<?endif;?>
								...
								<?if($arProperties["FIELD_NAME"] == "ORDER_PROP_2"):?>
								<? global $USER; ?>

								<div id="email-tip" class="topltip-error <?=($USER->IsAuthorized()) ? "autorized" : ''?>">
								<div class="close">X</div>
									<p>Пользователь с таким Email уже зарегестрирован. <a class="orange login" href="">Войти</a></p>
								</div>
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="Email"/>
									<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
										<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
									<?endif?>
								<?endif;?>

								<!-- если текущее свойство - Номер телефона  -->
								<?if($arProperties["FIELD_NAME"] == "ORDER_PROP_3"):?>
								<!--div id="confirm" class="desc-confirm">
									<p>Подтвердите номер телефона</p>
								</div-->
								<div id="order_phone" class="topltip-error <?=($USER->IsAuthorized()) ? "autorized" : ''?>">
									<div class="close">X</div>
										<p>Пользователь с таким телефоном уже зарегестрирован. <a class="orange login" href="">Войти</a></p>
									
								</div>
								<div class='desc-confirm'>
									<p>В течении 1 минуты вам на телефон придет код с подтверждением</p>
								</div>
								<div class='re-desc-confirm'>
									<p>Если вам не пришло сообщение, повторите отправку</p>
								</div>
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" data-tel="<?=$arProperties["VALUE"]?>" placeholder="Телефон"/>	
									<?
									
									if(!$USER->IsAuthorized()) { ?>
										<!--button id="check-phone" class="orange-btn">Подтвердить</button-->
										<input id="CONFIRM_PHOE" class="confirm-input" type="tel" name="CONFIRM_PHOE" value=""/>
										<button id="confirm-phone" class="orange-btn confirm-phone">ок</button>
										<button id="confirm-phone-success" class="orange-btn confirm-phone-success"><span class="glyphicon glyphicon-ok"></span></button>
										<button id="re-confirm" class="orange-btn re-confirm">
											<span class="glyphicon glyphicon-refresh"></span>
										</button>
										<input id="CONFIRM_PHONE_SUCCESS" type="hidden" name="CONFIRM_PHONE_SUCCESS" value="Y">

									<? } else if ($arProperties["CONFIRM_PHONE"] == "Y") { ?>
											<!--span class="confirm-tel">(подтвержден)</span-->
											<div class="btn-confirm inactive">
												<!--button id="check-phone" class="orange-btn">Подтвердить</button-->
												<input id="CONFIRM_PHOE" class="confirm-input" type="tel" name="CONFIRM_PHOE" value=""/>
												<button id="confirm-phone" class="orange-btn confirm-phone">ок</button>
												<button id="confirm-phone-success" class="orange-btn confirm-phone-success"><span class="glyphicon glyphicon-ok"></span></button>
									...
									<? } else if ($arProperties["CONFIRM_PHONE"] == "N"){ ?>
											<!--span class="confirm-tel inactive">(подтвержден)</span-->
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							?>
							<div class="bx_block r3x1">
								<textarea rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="Адрес"><?=$arProperties["VALUE"]?></textarea>
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
							</div>
