	...
	if (!!res.BASKET_DATA)
	{
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('sum_' + id))
					BX('sum_' + id).innerHTML = (item.FULL_PRICE_FORMATED != item.PRICE_FORMATED) ? item.FULL_PRICE*item.QUANTITY : item.SUM;
...
function recalcBasketAjax(params)
{
	...
	BX.ajax({
		...
		onsuccess: function(result)
		{
			...
			$('.bx_ordercart_order_table_container tr').each(function(){
				$(this).find('.custom');
			})
