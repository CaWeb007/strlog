...
{
    $commentTmp = HTMLToTxt(preg_replace(
        array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
        CSalePdf::prepareToPdf($params["BILL_COMMENT1"])
    ), '', array(), 0);
    $pdf->MultiCell(0, 10, $commentTmp, 0,'C');
    $pdf->Ln(15);
}
if ($params["BILL_COMMENT2"])
{
    ...
    $pdf->MultiCell(420, 10, $billNo_tmp, 0, 'C');
}

if ($params['BILL_PATH_TO_LOGO'])
{
    $pdf->Image($params['BILL_PATH_TO_LOGO'], $pageWidth - $margin['right'] - 100, $pdf->GetY() - 65,100, 65);
}
$pdf->Ln(5);
...
$x0 = $pdf->GetX();
$y0 = $pdf->GetY();
$x1 = $pageWidth - $margin['right'];
$y1 = $y0;
$pdf->Line($x0,$y0,$x1,$y1);
$pdf->Ln(0);

$pdf->SetFont($fontFamily, '', 9);
$text = CSalePdf::prepareToPdf($params['SELLER_COMPANY_BANK_NAME'].' г. '.$params['SELLER_COMPANY_BANK_CITY']);
while ($pdf->GetStringWidth($text) > 0)
{
    list($string, $text) = $pdf->splitString($text, 200);

    $pdf->Cell(300, 18, trim($string));
    if ($text)
        $pdf->Ln(10);
}
$pdf->Ln(12);
...
$pdf->SetFont($fontFamily, '', 7);
$pdf->Cell(300, 18, 'Банк получателя');
$y2 = $pdf->GetY();
$pdf->SetFont($fontFamily, '', 9);
$pdf->SetY($y0);
$pdf->SetX($x0 + 300);
$x2 = $pdf->GetX();
$pdf->Cell(50, 18, 'БИК');
$pdf->Cell(0, 18, $params['SELLER_COMPANY_BANK_BIC']);
$pdf->Ln(13);
$pdf->SetX($x2);
$pdf->Cell(50, 18, 'Сч. №');
$pdf->Cell(0, 18, $params['SELLER_COMPANY_BANK_ACCOUNT_CORR']);
$pdf->Ln(20);
$pdf->Cell(150, 18, 'ИНН   '.$params['SELLER_COMPANY_INN']);
$pdf->Cell(150, 18, 'КПП   '.$params['SELLER_COMPANY_KPP']);
$y3 = $pdf->GetY();
$pdf->Cell(50, 18, 'Сч. №');
$pdf->Cell(0, 18, $params['SELLER_COMPANY_BANK_ACCOUNT']);
$pdf->Ln(11);
$text = CSalePdf::prepareToPdf($params['SELLER_COMPANY_NAME']);
$pdf->Cell(300, 18, $text);
$pdf->Ln(15);
$pdf->SetFont($fontFamily, '', 7);
$pdf->Cell(300, 18, 'Получатель');
$pdf->Ln(0);

$pdf->Line($x0,$pdf->GetY() + 13 ,$x1,$pdf->GetY() + 13);
$pdf->Line($x0,$y2 + 13,$x1,$y2 + 13);
$pdf->Line($x0,$y3 + 14,$x0 + 300,$y3 + 14);
$pdf->Line($x0 + 150, $y3 + 2.5, $x0 + 150, $y3 + 13.5);
$pdf->Line($x2, $y0 + 15.5, $x2 + 50, $y0 + 15.5);
$pdf->Line($x0,$y0,$x0,$pdf->GetY() + 13);
$pdf->Line($x2,$y0,$x2,$pdf->GetY() + 13);
$pdf->Line($x2 + 50,$y0,$x2 + 50,$pdf->GetY() + 13);
$pdf->Line($x1,$y0,$x1,$pdf->GetY() + 13);
$pdf->Ln(21);

$pdf->SetFont($fontFamily, 'B', 12);
$pdf->Cell(300, 18, 'Счет на оплату заказа № '.$params['BILL_HEADER'].' от '.$params['BILL_ORDER_SUBJECT']);
$pdf->Ln();
$pdf->Line($x0,$pdf->GetY(),$x1,$pdf->GetY());
$pdf->Line($x0,$pdf->GetY()+0.5,$x1,$pdf->GetY()+0.5);

$strlog = '';
if ($params['SELLER_COMPANY_NAME']) $strlog.= $params['SELLER_COMPANY_NAME'];
if ($params['SELLER_COMPANY_INN']) $strlog.= ', ИНН '.$params['SELLER_COMPANY_INN'];
if ($params['SELLER_COMPANY_KPP']) $strlog.= ', КПП '.$params['SELLER_COMPANY_KPP'];
if ($params['SELLER_COMPANY_ADDRESS']) $strlog.= ', '.$params['SELLER_COMPANY_ADDRESS'];
if ($params['SELLER_COMPANY_PHONE']) $strlog.= ', тел: '.$params['SELLER_COMPANY_PHONE'];


$text = HTMLToTxt(preg_replace(
    array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
    CSalePdf::prepareToPdf($strlog)
), '', array(), 0);
//1
$pdf->Ln(12);
$pdf->SetFont($fontFamily, '', 9);
...
$pdf->SetFont($fontFamily, '', 9);
$pdf->SetY($pdf->GetY() + 9);
$pdf->Cell(100, 0, 'Грузоотправитель:');
$pdf->SetFont($fontFamily, 'B', 9);
$pdf->SetY($pdf->GetY() -10);
$pdf->SetX($pdf->GetX() + 85);
$pdf->MultiCell(0, 10, $text, 0, 'L');
...
$user = '';
if ($params['BUYER_PERSON_COMPANY_NAME']) $user.= $params['BUYER_PERSON_COMPANY_NAME'];
if ($params['BUYER_PERSON_COMPANY_INN']) $user.= ', ИНН '.$params['BUYER_PERSON_COMPANY_INN'];
if ($params['BUYER_PERSON_COMPANY_PHONE']) $user.= ', тел: '.$params['BUYER_PERSON_COMPANY_PHONE'];

$text = HTMLToTxt(preg_replace(
    array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
    CSalePdf::prepareToPdf($user)
), '', array(), 0);
$cellCount = 0;
$tmpT = $text;
while ($pdf->GetStringWidth($tmpT) > 0)
{
    list($string, $tmpT) = $pdf->splitString($tmpT, 500);
    $cellCount++;
}
$align = 4;
if ($cellCount === 2) $align = 10;
if ($cellCount === 3) $align = 15;

//3
$pdf->Ln(12);
$pdf->SetFont($fontFamily, '', 9);
$pdf->SetY($pdf->GetY() + 9);
$pdf->Cell(100, 0, 'Покупатель:');
$pdf->SetFont($fontFamily, 'B', 9);
$pdf->SetY($pdf->GetY() -$align);
$pdf->SetX($pdf->GetX() + 85);
$pdf->MultiCell(0, 10, $text, 0, 'L');
$pdf->Ln(12);
...
$pdf->SetFont($fontFamily, '', 9);
$pdf->SetY($pdf->GetY() + 9);
$pdf->Cell(100, 0, 'Грузополучатель:');
$pdf->SetFont($fontFamily, 'B', 9);
$pdf->SetY($pdf->GetY() - $align);
$pdf->SetX($pdf->GetX() + 85);
$pdf->MultiCell(0, 10, $text, 0, 'L');
...
$params['BILL_COLUMN_ARTICLE_TITLE'] = 'Артикул';
$params['BILL_COLUMN_ARTICLE_SORT'] = 2;
$params['BILL_COLUMN_ARTICLE_SHOW'] = 'Y';

$columnList = array('NUMBER','ARTICLE', 'NAME', 'QUANTITY', 'MEASURE', 'PRICE', 'SUM');
$arCols = array();
$vatRateColumn = 0;
...
if (count($params['BASKET_ITEMS']) > 0)
{
    ...
    foreach ($params['BASKET_ITEMS'] as $basketItem)
    {
        ...
        foreach ($arCols as $columnId => $col)
        {
            ...
            {
                case 'ARTICLE':
                    $data = CSalePdf::prepareToPdf($basketItem['ARTICLE']);
    ...
    if ($params['BILL_TOTAL_SHOW'] == 'Y')
    {
        ...
        $arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf('Всего к оплате');
        $arCells[$n][$arColumnKeys[$columnCount-1]] = CSalePdf::prepareToPdf(SaleFormatCurrency($params['SUM'], $params['CURRENCY'], true));
    }
...
do
{
    ...
    foreach ($arCols as $columnId => $column)
    {
        ...
        if ($vat > 0 || $columnId !== 'VAT_RATE')
            $pdf->Cell($arRowsWidth[$columnId], 10, $string, 0, 0, $k ? 'L' : 'C');
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
    ...
    do
    {
        ...
        foreach ($arCols as $columnId => $column)
        {
            ...
            if (in_array($columnId, array('QUANTITY', 'MEASURE')))
            {
                if (!is_null($arCells[$n][$columnId]))
                {
                    $pdf->Cell($rowWidth, 10, $string, 0, 0, 'R');
                }
            }
            elseif (in_array($columnId, array('PRICE', 'SUM')))
            {
                if (!is_null($arCells[$n][$columnId]))
                {
                    if ($n > $rowsCnt - 3){
                        $pdf->SetFont($fontFamily, 'B', $fontSize);
                        $pdf->Cell($rowWidth, 11, $string, 0, 0, 'R');
                        $pdf->SetFont($fontFamily, '', $fontSize);
                    }else{
                        $pdf->Cell($rowWidth, 10, $string, 0, 0, 'R');
                    }
            ...
            {
                if (!is_null($arCells[$n][$columnId]))
                    $pdf->Cell($rowWidth, 10, ($l == 0) ? $string : '', 0, 0, 'C');
            }
            elseif ($columnId == 'NAME')
            {
                if (!is_null($arCells[$n][$columnId]))
                    $pdf->Cell($rowWidth, 10, $string, 0, 0,  ($n > $cntBasketItem) ? 'R' : '');
            }
            ...
            else
            {
                if (!is_null($arCells[$n][$columnId]))
                {
                    $pdf->Cell($rowWidth, 10, $string, 0, 0,   ($n > $cntBasketItem) ? 'R' : 'L');
                }
            }
...
{
    $pdf->SetFont($fontFamily, '', $fontSize);
    $pdf->Write(10, CSalePdf::prepareToPdf(Loc::getMessage(
    ...
    {
        $pdf->Write(10, CSalePdf::prepareToPdf(Number2Word_Rus($params['SUM'])));
    }
    else
    {
        $pdf->Write(10, CSalePdf::prepareToPdf(strip_tags(SaleFormatCurrency(
            $params['SUM'],
            $params["CURRENCY"],
            false
        ))));
    ...
    $pdf->Ln(15);
    $pdf->Write(10, CSalePdf::prepareToPdf('Вес: '.$params['DELIVERY_WEIGHT']));
    $pdf->Ln(15);
    $x0 = $pdf->GetX();
    $x1 = $pageWidth - $margin['right'];
    $pdf->Line($x0,$pdf->GetY(),$x1,$pdf->GetY());
...
$pdf->Cell(55,0, 'Руководитель');
$widthLine = 147;
$pdf->Line($pdf->GetX() + 20,$pdf->GetY()+3,$pdf->GetX() + $widthLine,$pdf->GetY()+3);
$pdf->SetX($pdf->GetX() + $widthLine);
$pdf->Line($pdf->GetX() + 20,$pdf->GetY()+3,$pdf->GetX() + $widthLine,$pdf->GetY()+3);
$pdf->SetX($pdf->GetX() + $widthLine);
$pdf->Line($pdf->GetX() + 20,$pdf->GetY()+3,$pdf->GetX() + $widthLine,$pdf->GetY()+3);
$pdf->Ln(0);
...
$pdf->Write(15, 'должность');
$pdf->SetX($pdf->GetX() + 120);
$pdf->Write(15, 'подпись');
$pdf->SetX($pdf->GetX() + 96);
$pdf->Write(15, 'расшифровка подписи');
$pdf->Ln(30);

$pdf->SetFont($fontFamily,'B',$fontSize);
$pdf->Cell(55,0, 'Главный (старший) бухгалтер');
$widthLine = 147;
$pdf->SetX($pdf->GetX() + $widthLine);
$pdf->Line($pdf->GetX() + 20,$pdf->GetY()+3,$pdf->GetX() + $widthLine,$pdf->GetY()+3);
$pdf->SetX($pdf->GetX() + $widthLine);
...
$pdf->Image(
    ...
    $margin['left'] + 120, $pdf->GetY() - 60,
    110, 110
);

$pdf->SetFont($fontFamily,'',8);
$name = $params['SELLER_COMPANY_DIRECTOR_POSITION'];

if ($params['STORE_ID'] === 88){
    $name = $params['SELLER_COMPANY_ACCOUNTANT_NAME'];
}elseif($params['GROUP_ID'] === 10){
    //co kmc
    $name = $params['SELLER_COMPANY_DIRECTOR_NAME'];
}elseif($params['GROUP_ID'] === 11){
    //to
    $name = $params['SELLER_COMPANY_ACCOUNTANT_POSITION'];
}elseif($params['GROUP_ID'] === 12){
    //co pgs
    $name = $params['SELLER_COMPANY_DIRECTOR_NAME'];
}elseif($params['GROUP_ID'] === 14){
    //kp legal
    $name = $params['SELLER_COMPANY_DIRECTOR_POSITION'];
}
$pdf->SetXY($pdf->GetX() - 98, $pdf->GetY() - 10);
$pdf->Write(15, $name);
...
$trFileName = '/upload/tmp/scheta/'.CUtil::translit($fileName.rand(0,1000), 'ru', array('max_len' => 1024, 'safe_chars' => '.', 'replace_space' => '-'));

$t = $pdf->Output($dr.$trFileName, $dest, $fileName);
